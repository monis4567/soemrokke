...
#SBATCH -c 1
#SBATCH -t 1:00:00
#SBATCH -J 07A_sstacks
#SBATCH -o stdout_07A_sstacks.txt
#SBATCH -e stderr_07A_sstacks.txt
...
cp "${WD}"/"${PMAPF}" .
#mkdir "${D04a}"
# get a list of the files that has 'catalog' in the file name 
# in the "${D03}"/"${D03d}" directory
LSS=$(ls "${WD}"/"${D03}"/"${D03d}"/ | grep catalog)
#make the list  an array you can iterate over
declare -a SAR=($LSS)
# iteraste over the elements in this list
for file in ${SAR[@]}
do
	#move each 'catalog' file in to the 04_popmaps directory"
	mv "${WD}"/"${D03}"/"${D03d}"/$file "${WD}"/"${D04}"/.
done
# I discovered I had to move these 'catalog' files that were generated by the 'part06A_sbatch_run_cstacks.sh'
# code I ran prior to this present code
# if there were any '*tsv.gz' files not listed in the popmap file the sstacks operation could not be completed
# it appears the files :
# catalog.alleles.tsv.gz
# catalog.snps.tsv.gz
# catalog.tags.tsv.gz
# Originates from the  'part06A_sbatch_run_cstacks.sh' code I ran prior to this code
# So the above iteration implemented to move these 'catalog' files to a different folder, so that the 'sstacks'
# code here below does not stumble upon these *tsv.gz files being present in the input directory
...
sstacks -P "${WD}"/"${D03}"/"${D03d}"/ -M "${WD}"/"${D04}"/"${PMAPF}" -p 8
